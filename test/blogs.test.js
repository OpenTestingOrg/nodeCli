...
describe("User is not logged in", () => {
  const actions = [
    ...
    },
    {
      ...
      path: "/api/blogs",
      data: {
        title: "testing post function",
        content: "testing post content",
      },
    },
  ];
  test("All related actions are prohabited", async () => {
      const results = await page.execuRequest(actions);
      for(let result of results){
        expect(result).toEqual({error:'You must log in!'})
      }
  });
  //   test("User cannot create blog posts", async () => {
  //   const result =await page.post('/api/blogs',{title:"testing post function",content:"testing post content"})
  //     expect(result).toEqual({error:'You must log in!'})
  //   });
